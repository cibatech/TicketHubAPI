...
var PointId: string
var Service: GetTravelsByBeginningPointIdUseCase
...
describe("Get Travels by the Beginning Point Id Service: Good Case", async () => {
    beforeEach(async () => {
        ...
        Service = new GetTravelsByBeginningPointIdUseCase(TravelRepo, PointRepo)
    ...
    it("Should be able to get Travels by the Beginning Point Id", async () => {
        ...
        expect(travels[0].BeginningPointId).toBe(PointId)
    })
})

describe("Get Travels by the Beginning Point Id Service: Bad Case", async () => {
    beforeEach(async () => {
        PointRepo = new InMemoryPointRepository()
        TravelRepo = new InMemoryTravelRepository()
        Service = new GetTravelsByBeginningPointIdUseCase(TravelRepo, PointRepo)
    ...
    it("Should throw EntityDoesNotExistsError if Beginning Point Id does not exists", async () => {
